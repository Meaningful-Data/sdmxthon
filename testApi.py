...
def main():
    ...
    # dsd = first_element_dict(read_sdmx(url_bis, validate=False).payload['DataStructures'])
    ...
    structure = DataStructureDefinition(id_="BOP", version="1.0")
    dataset = Dataset(structure=structure, data=df)
    dataset.to_xml(message_type=MessageTypeEnum.StructureDataSet, outputPath="BOP.xml")
    df2 = first_element_dict(get_pandas_df("BOP.xml"))
